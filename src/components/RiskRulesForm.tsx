...
export function RiskRulesForm() {
  ...
  const [engineVer, setEngineVer] = useState<'v1'|'v2'|'v3'|'v4'|'v5'|'v6'|'v7'|'v8'>('v1');
  ...
  const changeEngine = async (ver: 'v1'|'v2'|'v3'|'v4'|'v5'|'v6'|'v7'|'v8') => {
    ...
    try {
      ...
      if (res.ok) {
        setEngineVer(ver);
        const label = ver === 'v8' ? 'V8 Weekend Grid Bot' : ver === 'v7' ? 'V7 Smart Grid Bot' : ver === 'v6' ? 'V6 Smart Grid Bot' : ver === 'v5' ? 'V5 Macro Day-Trader' : ver === 'v4' ? 'V4 Smart Aggressive' : ver === 'v3' ? 'V3 Sniper' : ver === 'v2' ? 'V2 SMC' : 'V1 Classic';
           ...
           <div className="p-6 border-t border-[#1a2540] space-y-8">
              ...
              <div className={`p-4 rounded-xl border-2 ${simulatedCapital > 0 ? 'border-amber-500/60 bg-amber-500/5' : 'border-[#1a2540] bg-[#0A0E1A]'}`}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div 
                    onClick={() => changeEngine('v5')}
                    className={`p-3 rounded-lg border-2 cursor-pointer transition-all duration-300 ${
                      engineVer === 'v5' ? 'border-[#ef4444] bg-[#ef4444]/10 shadow-[0_0_15px_rgba(239,68,68,0.2)]' : 'border-gray-800 hover:border-gray-700'
                    }`}
                  >
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`font-bold ${
                          engineVer === 'v5' ? 'bg-gradient-to-r from-[#ef4444] to-[#f43f5e] text-transparent bg-clip-text' : 'text-gray-300'
                        }`}>V5 Macro Day-Trader</span>
                      </div>
                      <div className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                        engineVer === 'v5' ? 'bg-gradient-to-r from-[#ef4444] to-[#f43f5e] text-white' : 'text-gray-400 hover:text-white'
                      }`}>
                        DAY TRADING
                      </div>
                    </div>
                ...
                {/* V7 GRID BOT */}
                <div 
                  onClick={() => changeEngine('v7')}
                  className={`mt-3 p-3 rounded-lg border-2 cursor-pointer transition-all duration-300 ${
                    engineVer === 'v7' ? 'border-[#3b82f6] bg-[#3b82f6]/10 shadow-[0_0_15px_rgba(59,130,246,0.2)]' : 'border-gray-800 hover:border-gray-700'
                  }`}
                >
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      <span className="text-xl">🔷</span>
                      <span className={`font-bold ${
                        engineVer === 'v7' ? 'bg-gradient-to-r from-[#3b82f6] to-[#2563eb] text-transparent bg-clip-text' : 'text-gray-300'
                      }`}>V7 Smart Grid Bot</span>
                      {engineVer === 'v7' && <span className="text-[10px] text-blue-300 bg-blue-900/50 px-1.5 py-0.5 rounded">OPTIMAL</span>}
                    </div>
                    <div className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                      engineVer === 'v7' ? 'bg-gradient-to-r from-[#3b82f6] to-[#2563eb] text-white' : 'text-gray-400 hover:text-white'
                    }`}>
                      NO AI
                    </div>
                  </div>
                  <p className="text-xs text-gray-500">15x leverage · 8 grids · 0.5% spacing. <span className="text-blue-400/80">Soft Expand</span> saat harga keluar range — tidak pernah close posisi otomatis. Tanpa SL, tanpa circuit breaker.</p>
                </div>

                {/* V8 WEEKEND GRID BOT */}
                <div 
                  onClick={() => changeEngine('v8')}
                  className={`mt-3 p-3 rounded-lg border-2 cursor-pointer transition-all duration-300 ${
                    engineVer === 'v8' ? 'border-[#8b5cf6] bg-[#8b5cf6]/10 shadow-[0_0_15px_rgba(139,92,246,0.2)]' : 'border-gray-800 hover:border-gray-700'
                  }`}
                >
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      <span className="text-xl">🟦</span>
                      <span className={`font-bold ${
                        engineVer === 'v8' ? 'bg-gradient-to-r from-[#8b5cf6] to-[#7c3aed] text-transparent bg-clip-text' : 'text-gray-300'
                      }`}>V8 Weekend Grid Bot</span>
                      {engineVer === 'v8' && <span className="text-[10px] text-purple-300 bg-purple-900/50 px-1.5 py-0.5 rounded">WEEKEND</span>}
                    </div>
                    <div className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                      engineVer === 'v8' ? 'bg-gradient-to-r from-[#8b5cf6] to-[#7c3aed] text-white' : 'text-gray-400 hover:text-white'
                    }`}>
                      NO AI
                    </div>
                  </div>
