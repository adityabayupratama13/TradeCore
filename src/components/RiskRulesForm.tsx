...
export function RiskRulesForm() {
  ...
  const isCustom = activeMode !== 'CUSTOM' && (
    ...
    // Simple heuristic to show "ðŸ”§ CUSTOM" if overridden
  );
  ...
  const applyMode = async (m: string, confirmed = false) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        showToastMsg(`âœ… Mode berhasil diubah ke ${TRADING_MODES[m].badge}. Semua parameter diperbarui otomatis.`, 'success');
        refreshStatus();
        if (m === 'DEGEN') setShowDegenModal(false);
      } else {
        showToastMsg(`âŒ Failed to change mode: ${data.message || 'Error'}`, 'error');
      }
    } catch(e: any) {
      showToastMsg(`âŒ Failed to connect: ${e.message}`, 'error');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const changeEngine = async (ver: 'v1'|'v2') => {
    ...
    try {
      ...
      if (res.ok) {
        setEngineVer(ver);
        showToastMsg(`âœ… AI Engine diubah ke ${ver === 'v2' ? 'V2' : 'V1'}`, 'success');
      }
    } catch(e) {
      showToastMsg(`âŒ Gagal mengubah AI Engine`, 'error');
    } finally {
  ...
  const saveOverrides = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        showToastMsg("âœ… Override & Target berhasil disimpan", 'success');
        refreshStatus();
      } else {
        showToastMsg(`âŒ Failed to save: ${data.error || 'Unknown error'}`, 'error');
      }
    } catch(e: any) {
      showToastMsg(`âŒ Failed to save: ${e.message}`, 'error');
    } finally {
      setIsSaving(false);
    }
  };
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
           <div className="bg-[#0A0E1A] border border-red-500/50 rounded-xl p-6 max-w-md w-full shadow-2xl shadow-red-900/20">
              <h3 className="text-xl font-bold text-red-500 mb-4 flex items-center gap-2">
                <AlertTriangle /> ðŸ’€ Aktivasi DEGEN Mode
              </h3>
      ...
      {capital <= 50 && (
        <div className="bg-emerald-900/30 border border-emerald-500/50 rounded-xl p-5 flex items-start gap-4 shadow-lg shadow-emerald-900/10">
           ...
           <div className="flex-1">
              ...
              <p className="text-emerald-100/80 text-sm mb-3">Berdasarkan capital ${(capital).toLocaleString()}, kami sarankan mulai dengan <span className="font-bold text-emerald-300">ðŸ›¡ï¸ SAFE MODE</span>.</p>
              <ul className="text-xs text-emerald-100/60 space-y-1 mb-4">
                 <li>â€¢ Loss per trade maksimal ${(capital * 0.03).toFixed(2)} (3%)</li>
                 <li>â€¢ Leverage rendah = tidak mudah liquidasi</li>
                 <li>â€¢ Validasi sistem dulu selama 1 bulan</li>
                 <li>â€¢ Kalau konsisten profit â†’ upgrade ke BALANCED</li>
              </ul>
              <button 
        ...
        <div className="lg:col-span-2 bg-[#0E1628] border border-[#1a2540] rounded-xl p-6 shadow-xl">
           <h3 className="text-lg font-bold text-white mb-4 border-b border-[#1a2540] pb-2 flex items-center gap-2">
              <span style={{color: isCustom ? '#aaa' : getModeConfig(activeMode).color}}>
                {isCustom ? 'ðŸ”§ CUSTOM MODE ' : getModeConfig(activeMode).badge}
              </span>
              <span className="text-gray-500 text-sm uppercase font-normal">â€” Active Settings</span>
           </h3>
           ...
           <div className="space-y-4">
            ...
            <div className="mt-4 pt-4 border-t border-green-900/30">
               <div className="bg-green-950/20 p-3 rounded border border-green-900/50 flex justify-between items-center">
                 <span className="text-gray-400 text-sm font-bold flex items-center gap-1">ðŸŽ¯ Daily Profit Target</span>
      ...
      <div className="bg-[#0E1628] border border-[#1a2540] rounded-xl overflow-hidden shadow-xl">
         ...
         >
           <div className="flex items-center gap-2 text-gray-300 font-bold">
              <TrendingDown className="w-4 h-4" /> âš™ï¸ Custom Override (Advanced)
           </div>
              ...
              <div className={`p-4 rounded-xl border-2 ${simulatedCapital > 0 ? 'border-amber-500/60 bg-amber-500/5' : 'border-[#1a2540] bg-[#0A0E1A]'}`}>
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-amber-400 flex items-center gap-2">
                      ...
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-black ${simulatedCapital > 0 ? 'bg-amber-500 text-black' : 'bg-[#1a2540] text-gray-500'}`}>
                        {simulatedCapital > 0 ? `ACTIVE â€“ $${simulatedCapital}` : 'OFF'}
                      </span>
                    </h4>
                ...
                {simulatedCapital > 0 && (
                  <p className="text-[10px] text-amber-400/70 mt-2">
                    âš¡ Position size dihitung dari ${simulatedCapital} â€” bukan dari real balance Binance
                  </p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                 ...
                 <div className="space-y-4">
                    ...
                    <div className="flex flex-col gap-1 mb-3">
                      <label className="text-xs text-emerald-400 font-bold flex items-center gap-1">ðŸŽ¯ Daily Profit Target (USD)</label>
                    ...
                    <div className="flex flex-col gap-1">
                     ...
                     <div className="flex flex-col gap-1">
                       <label className="text-xs text-orange-400/70">â±ï¸ Max Hold Profitable Trade (jam)</label>
                     ...
                     <div className="flex flex-col gap-1">
                        <label className="text-xs text-yellow-400/80">âš¡ Max Entry Drift % (stale signal filter)</label>
                     ...
                     <div className="flex flex-col gap-1">
                        <label className="text-xs text-purple-400/80">ðŸ”„ Max Trades Per Symbol / Hari</label>
